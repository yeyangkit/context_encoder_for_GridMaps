...
class ContextEncoder():
    ...
    def build_generator(self):
        ...
        model.add(Conv2D(gf * 8, kernel_size=3, strides=2, padding="same"))
        model.add(LeakyReLU(alpha=0.2))
        model.add(BatchNormalization(momentum=0.8))

        model.add(Conv2D(gf * 16, kernel_size=3, strides=2, padding="same"))
        model.add(LeakyReLU(alpha=0.2))
        model.add(BatchNormalization(momentum=0.8))

        model.add(Conv2D(gf * 32, kernel_size=3, strides=2, padding="same"))
        model.add(LeakyReLU(alpha=0.2))
        model.add(BatchNormalization(momentum=0.8))

        model.add(Conv2D(gf * 64, kernel_size=3, strides=2, padding="same"))
        model.add(LeakyReLU(alpha=0.2))
        model.add(Dropout(0.5)) # ONLY HERE DROPOUT AND IN ORIGINAL CODE IS IMPLEMENTED WITH 512 WHICH  SPRINGS 256
    ...
    def mask_randomly(self, imgs):
        y1 = np.random.randint(low=FLAGS.frame, high=(self.img_rows - self.mask_height - FLAGS.frame), size=imgs.shape[0])
        y2 = y1 + self.mask_height
        x1 = np.random.randint(low=FLAGS.frame, high=(self.img_rows - self.mask_width - FLAGS.frame), size=imgs.shape[0])
    ...
    def train(self, epochs, batch_size, sample_interval=50):
        ...
        for epoch in range(epochs):
            ...
            if epoch % sample_interval == 0:
                ...
                inpainted_imgs_sample = np.zeros((samples, self.img_rows, self.img_cols))
                inpainted_imgs_sample= inpainted_imgs_sample.reshape(samples, self.img_rows, self.img_cols)
                ...
                for k in range(samples):
                    ...
                    # plot missing
                    missing_imgs_sample = np.zeros((samples, self.img_rows, self.img_cols))
                ...
                for k in range(samples):
                    ...
                    axs2[0, k].imshow(cut_imgs_sample[k].reshape(2*frame+self.mask_height, 2*frame+self.mask_width), cmap='gray')
                    axs2[0, k].axis('off')
                    ...
                    axs2[1, k].axis('off')
                fig2.savefig("%s/%d_1.png" % (FLAGS.Pred_img_dir, epoch))
                plt.close()
...
if __name__ == '__main__':
    ...
    tf.app.flags.DEFINE_string('Pred_img_dir', 'InpaintingImagesComparision/Epoch20_Intervall1_batchSize16_frame16-NOTdefault8_deeper',
    ...
    tf.app.flags.DEFINE_integer('frame', 16,  #
                                """ width of the frame for visualization """)
    ...
    context_encoder.train(epochs=20, batch_size=FLAGS.Batch_size,
